...
void InitParameters(double *qeqchi, double *qeqmui, int *qeqids, int MaxElement,
                    char *fname) {
  FILE *fp;
...
void PrintParameters(double *qeqchi, double *qeqmui, double *qeqrad,
                     int *qeqids, int MaxElement) {
  int i;
  printf("Element  Electronegativity chi(eV)  Hardness 0.5J(eV) SIZE(A)\n");
  for (i = 0; i < MaxElement; i++)
    printf("%4d   %12.8f   %12.8f %12.8f\n", qeqids[i], qeqchi[i], qeqmui[i],
           qeqrad[i]);
  return;
}
