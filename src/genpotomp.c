...
void genpot(double *av1, double *av2, double *av3, int natoms, int *atoms_num,
  ...
  for (iatom = 0; iatom < natoms; iatom++) {
    ...
    /*		zeta_i =
     * 0.5*qeqlambda*(2.0*iatom_ni+1.0)/qeqrad[atoms_num[iatom]-1]
     * ;
     */
    ...
    for (jatom = 0; jatom <= iatom; jatom++) {
       ...
       */
      /*			zeta_j =
       * 0.5*qeqlambda*(2.0*jatom_nj+1.0)/qeqrad[atoms_num[jatom]-1];  */
...
void genpot_on_grid(double *av1, double *av2, double *av3, int natoms,
  ...
  if (av2sz < avecszmin) avecszmin = av2sz;
  if (av3sz < avecszmin) avecszmin = av3sz;
#pragma omp parallel for private(ip, xx, yy, zz, iatom, dx, dy, dz, vijre,   \
                                 vijR)                                       \
    shared(gridnl, L1, L2, L3, h1, h2, h3, pot, natoms, atoms_xyz, av1, av2, \
           av3, avecszmin, dvREAL, eta, radmax, bgridx, bgridy, bgridz,      \
           bgridcoef, kvectors, atoms_charge)
