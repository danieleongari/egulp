...
void InitEconfig(tConfigure *confptr, char *fname) {
  ...
  if (fp == NULL) {
    ...
    fscanf(fp, "%s %s %s %s %s %s %s %s %s %s", tmpstr1, tmpstr2,
           confptr->input_grid_file, tmpstr4, tmpstr5, tmpstr6, tmpstr7,
           tmpstr8, tmpstr9, tmpstr10);
    confptr->build_grid_from_scratch = atoi(tmpstr2);
    printf("build_grid_from_scratch is read as %d\n",
           confptr->build_grid_from_scratch);
    if (!((confptr->build_grid_from_scratch == 0) ||
          (confptr->build_grid_from_scratch == 1))) {
      printf(
          "invalid build_grid_from_scratch=%d. Should be 1(True) or 0(False)\n",
          confptr->build_grid_from_scratch);
      exit(-1);
    }
    ...
    if (confptr->offset == -1)
      printf("All region minus vdw will be used if use_vdw_factor==0\n");

    if ((confptr->build_grid == 1) && (confptr->build_grid_from_scratch == 0)) {
      printf("grid will be loaded from file %s\n", confptr->input_grid_file);
      printf(
          "resolution vdw_factors and offsets will be read from %s, not from "
          "the configure file\n",
          confptr->input_grid_file);
    }
    ...
    if ((confptr->build_grid == 1) && (confptr->save_grid == 1))
      printf("grid will be saved into file %s\n", confptr->output_grid_file);

    fscanf(fp, "%s %s", tmpstr1, tmpstr2);
    confptr->calculate_pot_diff = atoi(tmpstr2);
    if (!((confptr->calculate_pot_diff == 0) ||
          (confptr->calculate_pot_diff == 1))) {
      printf("invalid calculate_pot_diff =%d. Should be 1(True) or 0(False)\n",
             confptr->calculate_pot_diff);
      exit(-1);
    }
    if (confptr->calculate_pot_diff == 1)
      printf(
          "electrostatic potential based on the output-input charges will be "
          "constructed\n");
    else
      printf(
          "electrostatic potential based on the output-input charges will not "
          "be constructed\n");

    fscanf(fp, "%s %s %s", tmpstr1, tmpstr2, confptr->output_pot_file);
    confptr->calculate_pot = atoi(tmpstr2);
    if (!((confptr->calculate_pot == 1) || (confptr->calculate_pot == 2) ||
          (confptr->calculate_pot == 0))) {
      printf(
          "invalid calculate_pot =%d. Should be (1,2 - calculate input,output "
          "or 0 - do not calculate)\n",
          confptr->calculate_pot);
      exit(-1);
    }
    if (confptr->calculate_pot == 1)
      printf(
          "electrostatic potential based on the input  charges will be "
          "constructed\n");
    else if (confptr->calculate_pot == 2)
      printf(
          "electrostatic potential based on the output  charges will be "
          "constructed\n");
    else
      printf(
          "electrostatic potential based on the input  or output charges will "
          "not be constructed\n");
    if ((confptr->calculate_pot == 1) || (confptr->calculate_pot == 2))
      printf("electrostatic potential will be saved in file %s\n",
    ...
    if (confptr->skip_everything == 1)
      printf("calculation of charges and potentials will be skipped\n");
    else
      printf("calculation of charges and potentials will not be skipped\n");

    fscanf(fp, "%s %s", tmpstr1, tmpstr2);
    confptr->point_charges_present = atoi(tmpstr2);
    printf("point_charges_present is read as %d\n",
           confptr->point_charges_present);
    if (!((confptr->point_charges_present == 0) ||
          (confptr->point_charges_present == 1))) {
      printf(
          "invalid point_charges_present  =%d. Should be 1(True) or 0(False)\n",
          confptr->point_charges_present);
      exit(-1);
    }
    if (confptr->point_charges_present == 1)
      printf("point charges are present...\n");
    else
      printf("point charges are not present\n");
    ...
    if (confptr->imethod == 1)
      printf("SPLIT CHARGE CALCULATION\n");
    else if (confptr->imethod == 0)
      printf("QEQ CALCULATION\n");
    else
      printf("INVALID METHOD OPTION...\n");
    fclose(fp);
  }
  return;
}

void DestroyEconfig(tConfigure conf) { return; }
