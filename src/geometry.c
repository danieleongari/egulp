	 ...
	 transfo[6] = geometry->av1[2]; 
         ...
         for(iatom = 0; iatom < geometry->natoms; iatom++) { 
	      ...
	      if(mgamma < 0.0) mgamma = mgamma+1.0; 
              if(la || lb || lg) printf("atom %3d new natural coordinates %12.8f %12.8f %12.8f\n", iatom+1, malpha, mbeta, mgamma);       
 
	      xx = geometry->atoms_xyz[3*iatom+0] = transfo[0]*malpha+transfo[1]*mbeta+transfo[2]*mgamma;
	      yy = geometry->atoms_xyz[3*iatom+1] = transfo[3]*malpha+transfo[4]*mbeta+transfo[5]*mgamma; 
              zz = geometry->atoms_xyz[3*iatom+2] = transfo[6]*malpha+transfo[7]*mbeta+transfo[8]*mgamma; 
	 ...
	 transfo[6] = geometry->av1[2]; 
         ...
         for(iatom = 0; iatom < geometry->natoms; iatom++) { 
              ...
              if (la || lb || lg) { 
                 ...
                 printf("atom %3d inp natural coordinates %12.8f %12.8f %12.8f\n", iatom+1, alpha, beta, gamma);    
		 printf("atom %3d is not mapped into the central cell\n", iatom+1);
		 if(la) { printf("natural coordinate 1: %15.12f\n", alpha); }
		 if(lb) { printf("natural coordinate 2: %15.12f\n", beta);  }
		 if(lg) { printf("natural coordinate 3: %15.12f\n", gamma);  }
...
void InitBonds(tGeometry *geometry) 
{
       ...
       num_distinct_el = 0; 
       for(iatom = 0; iatom < 103; iatom++) num_distinct_el = num_distinct_el+elements[iatom]; 
       ...
       allocate_int_array(&(geometry->tbonds_indexij), 2*btmax, "can not allocate tbonds_indexij\n"); 
       for(ibondt = 0; ibondt < btmax; ibondt++) geometry->tbonds_indexij[2*ibondt+0] = geometry->tbonds_indexij[2*ibondt+1] = 0; 
       geometry->tbonds = 0; 

       /* SetVDWRADIUS(vdw);   */ 
       /* Covalent radius is in A? */
       SetCOVRADIUS(vdw); 
       for(iatom = 0; iatom < 103; iatom++) vdw[iatom] = 1.2*vdw[iatom]/0.5291772; 
       ...
       for(iatom = 0; iatom < geometry->natoms; iatom++) { 
		...
		for(icell = -1; icell <= 1; icell++) { 
	        ...
	        for(kcell = -1; kcell <= 1; kcell++) { 
			          xatomj = (geometry->atoms_xyz)[jatom*3+0]+icell*(geometry->av1[0])+jcell*(geometry->av2[0])+kcell*(geometry->av3[0]);
                                  yatomj = (geometry->atoms_xyz)[jatom*3+1]+icell*(geometry->av1[1])+jcell*(geometry->av2[1])+kcell*(geometry->av3[1]); 
			          zatomj = (geometry->atoms_xyz)[jatom*3+2]+icell*(geometry->av1[2])+jcell*(geometry->av2[2])+kcell*(geometry->av3[2]); 
				  rij = sqrt( (xatomi-xatomj)*(xatomi-xatomj)+(yatomi-yatomj)*(yatomi-yatomj)+(zatomi-zatomj)*(zatomi-zatomj)  ); 
				  if(rij < 1.0e-8) { 
				  	printf("atomj=%d maps into atomi=%d under primitive translation %d %d %d\n", iatom+1, jatom+1, icell, jcell, kcell); 
					exit(-1); 
				  } 
				  ...
				  rijvdw = (vdw[niatom-1]+vdw[njatom-1])*(AUTOANG);
		                  if(rij < rijvdw ) { 
		                       ...
		                       for(ibondt = 0; ibondt < geometry->tbonds; ibondt++)  { 
		                            lii = ((geometry->tbonds_indexij[2*ibondt+0] == niatom) && (geometry->tbonds_indexij[2*ibondt+1] == njatom)); 
			                    ljj = ((geometry->tbonds_indexij[2*ibondt+0] == njatom) && (geometry->tbonds_indexij[2*ibondt+1] == niatom)); 
			                    if(lii || ljj) { l_bond_type_found = 1; break; } 
		                       }
	...
	for(ibondt = 0; ibondt < geometry->tbonds; ibondt++) 
		printf("bond type number %4d: atom types (%4d,%4d)\n", ibondt+1, geometry->tbonds_indexij[2*ibondt+0],  geometry->tbonds_indexij[2*ibondt+1]); 
	...
	if(geometry->nbonds < 1) { 
       ...
       allocate_int_array(&(geometry->nbonds_indexij), 2*geometry->nbonds, "can not allocate nbonds_indexij\n"); 
       allocate_int_array(&(geometry->nbonds_type),  geometry->nbonds, "can not allocate nbonds_type\n"); 
...
void PrintBondInfo(tGeometry *geometry)
{
	...
	for(ibond = 0; ibond < geometry->nbonds; ibond++) 
	     printf("bond %10d: %4d %4d\n", ibond+1, geometry->nbonds_indexij[2*ibond+0],  geometry->nbonds_indexij[2*ibond+1]); 
        return; 
} 
...
{ 
	 ...
	 if( n == 2) {  
		...
		a32 = mat[3*2+1]; 
		a33 = mat[3*2+2]; 
	        res = a11*(a33*a22-a32*a23)-a21*(a33*a12-a32*a13)+a31*(a23*a12-a22*a13); 
	 } else  { 
