...
void  InitESEQParams(double *qeqchi,  double *qeqmui, int *qeqids, int MaxElement, char *fname, tESEQParams *eseqptr)
{
	 ...
	 for(i = 0; i   < MaxParam; i++) { 
	   ...
	   if( (eseqptr->number_of_bond_types < 1) || (eseqptr->number_of_bond_types >  MaxPairs))  {
	        printf("Invalid number of bond types %d...\n", eseqptr->number_of_bond_types); 
		printf("should be in the (1..%d) range\n", MaxPairs); 
		exit(-1); 
	    } 
	 
	    allocate_int_array(&(eseqptr->bond_atom_types), 2*eseqptr->number_of_bond_types, 
	       "can not allocate bond_atom_types\n"); 
	    allocate_double_array(&(eseqptr->elneg_bondcorr), eseqptr->number_of_bond_types, 
	       "can not allocate elneg_bondcorr\n"); 
	    allocate_double_array(&(eseqptr->hard_bondcorr),  eseqptr->number_of_bond_types, 
	       "can not allocate hard_bondcorr\n"); 

           /* the same parameters but in the geometry order */ 
	    allocate_double_array(&(eseqptr->elneg_bondcorr_geom), eseqptr->number_of_bond_types, 
	       "can not allocate elneg_bondcorr_geom\n"); 
	    allocate_double_array(&(eseqptr->hard_bondcorr_geom),  eseqptr->number_of_bond_types, 
		 ...
		 if(ElNum1 < ElNum2) { 
		  ...
		  eseqptr->hard_bondcorr[i] = atof(tmpstr4); 
	       ...
	       for(itype = 0; itype < eseqptr->number_of_bond_types; itype++) 
		       ...
		       if ( (eseqptr->bond_atom_types[2*itype] == eseqptr->bond_atom_types[2*jtype]) && 
		              (eseqptr->bond_atom_types[2*itype+1] == eseqptr->bond_atom_types[2*jtype]+1) ) { 
...
{ 
        int ibond, iatom; 
        int atom_i, atom_j; 
	allocate_double_array(&(eseqptr->TMatrix), ncharges*nbonds, "can not allocate TMatrix\n"); 
	allocate_double_array(&(eseqptr->TMatrixPINV), ncharges*nbonds, "can not allocate TMatrixPINV\n"); 
	for (iatom = 0; iatom < ncharges; iatom++) { 
            for (ibond = 0; ibond < nbonds; ibond++) { 
                  ...
                  if((iatom+1) == atom_i) eseqptr->TMatrix[iatom*nbonds+ibond] = 1.0; 
		  else if ((iatom+1) == atom_j) eseqptr->TMatrix[iatom*nbonds+ibond] = -1.0;
	          else eseqptr->TMatrix[iatom*nbonds+ibond] = 0.0;        
...
void BuildParamsGEOOrder(tESEQParams *eseqptr,  int tbonds, int *tbonds_indexij) {
       ...
       for(geo_bondt = 0; geo_bondt < tbonds; geo_bondt++) { 
		...
		for (inp_bondt = 0; inp_bondt < eseqptr->number_of_bond_types; inp_bondt++) { 
			if( (eseqptr->bond_atom_types[2*inp_bondt+0] == tbonds_indexij[2*geo_bondt+0]) &&
			      (eseqptr->bond_atom_types[2*inp_bondt+1] == tbonds_indexij[2*geo_bondt+1])) {  
			       l_bond_type_found = 1; 
			       eseqptr->elneg_bondcorr_geom[geo_bondt] = eseqptr->elneg_bondcorr[inp_bondt]; 
			       eseqptr->hard_bondcorr_geom[geo_bondt] = eseqptr->hard_bondcorr[inp_bondt]; 
			       break; 
                         }
		}
		if(l_bond_type_found  != 1) { 
		      printf("geometry bond of type (%d %d) is not found among input parameters\n", 
...
void BuildBondHCorrections(tESEQParams *eseqptr,  int natoms, int nbonds,  int *nbonds_indexij, 
       ...
       for(ibond = 0; ibond < nbonds; ibond++) { 
              ...
              for(jbond = 0; jbond < nbonds; jbond++) {  
	           if(ibond == jbond) Jprime[ibond*nbonds+jbond]  = eseqptr->hard_bondcorr_geom[bond_type]; 
		   else Jprime[ibond*nbonds+jbond] = 0.0;
	      }	    
       } 	       
       ...
       for(ibond = 0; ibond < nbonds; ibond++) {  
          for(icharge = 0; icharge < natoms; icharge++) {  
	       sm = 0.0; 
	       for(k = 0; k < nbonds; k++) sm = sm + Jprime[ibond*nbonds+k]*eseqptr->TMatrixPINV[k*natoms+icharge]; 
       ...
       for(icharge = 0; icharge < natoms; icharge++) { 
              for(jcharge = 0; jcharge < natoms; jcharge++)  {
                    sm = 0.0; 
	           for (k =  0; k < nbonds; k++)  sm = sm + (eseqptr->TMatrixPINV[k*natoms+icharge])*JT[k*natoms+jcharge]; 
            ...
            int *nbonds_type, int tbonds,  int *tbonds_indexij, double *BondE)
{
      ...
      for(icharge = 0; icharge < natoms; icharge++) { 
		...
		electroneg = eseqptr->elneg_bondcorr_geom[bond_type]; 
	        BondE[icharge] =  BondE[icharge] + electroneg*eseqptr->TMatrix[icharge*nbonds+ibonds]; 
            } 
      } 
      return; 
}
