...
void InitEGULPScf(tSCF *SCF, double *atoms_charge, int *atom_num,
  ...
  allocate_double_array(&(SCF->atoms_charge), SCF->natoms,
                        "can not allocate atom_charge in SCF\n");
  allocate_double_array(&(SCF->atoms_charge_old), SCF->natoms,
                        "can not allocate atom_charge_old in SCF\n");
  ...
  allocate_double_array(&(SCF->energy), SCF->niter,
                        "can not allocate energy in SCF\n");
  for (i = 0; i < SCF->niter; i++) SCF->energy[i] = 0.0;
  allocate_double_array(&(SCF->QEQ_JArray), (SCF->natoms) * (SCF->natoms),
                        "can not allocate energy in SCF\n");
  allocate_double_array(&(SCF->QEQ_ChiArray), SCF->natoms,
                        "can not allocate energy in SCF\n");
  for (i = 0; i < SCF->natoms; i++) {
    SCF->QEQ_ChiArray[i] = 0.0;
    for (j = 0; j < SCF->natoms; j++)
      SCF->QEQ_JArray[i * SCF->natoms + j] = 0.0;
  }
  return;
}
...
double getQEQEnergy(int natoms, double *atoms_charge, double *JArray,
  ...
  for (i = 0; i < natoms; i++) {
    ...
    for (j = 0; j < natoms; j++)
      res = res +
            0.5 * JArray[i * natoms + j] * atoms_charge[i] * atoms_charge[j];
  }
  return (res);
}
