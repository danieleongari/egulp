...
int prepareKSUM(double GMax, double eta, int NUMR, 
      double *b1,  double *b2,  double *b3,  
      double *bgridx, double *bgridy, double *bgridz,  double *bgridcoef)
{      
	int GVEC = 0; 
		...
		for (k2=-NUMR; k2 <= NUMR; k2++) { 
				...
				if((k1 == 0) && (k2 == 0) && (k3 == 0)) continue; 
		       		...
		       		bvec[2] = k1*b1[2]+k2*b2[2]+k3*b3[2];
	               		bvecsz = sqrt(bvec[0]*bvec[0]+bvec[1]*bvec[1]+bvec[2]*bvec[2]);  
		       		if(bvecsz  < GMax ) { 
					GVEC = GVEC + 1; 
...
void qmatrixelement(double dx, double dy, double dz, double *av1, double *av2, double *av3, 
                             double V, double eta,   double radmax, double tweatpi, 
                             double *bgridx, double *bgridy, double *bgridz, double *bgridcoef, 
			     int kvectors, double *vijre,  double *vijself,  double *vijR ) 
{ 
	double sumre;
        int k; 
	int lzdisp;
	...
	for (k  = 0; k < kvectors; k++) 
		sumre = sumre + cos(dx*bgridx[k]+dy*bgridy[k]+dz*bgridz[k])*bgridcoef[k]; 
			...
			for (t3 = -numr; t3 <= numr; t3++) { 
				if ((lzdisp==1) && (t1 == 0) && (t2 == 0) && (t3 == 0)) continue; 
		       		...
		       		tvec[2] = t1*av1[2]+t2*av2[2]+t3*av3[2];
	               		tvecsz = sqrt(tvec[0]*tvec[0]+tvec[1]*tvec[1]+tvec[2]*tvec[2]); 
				drx = dx + tvec[0];
				dry = dy + tvec[1];
				drz = dz + tvec[2];
				drr = sqrt(drx*drx + dry*dry+drz*drz); 
				if(drr  < radmax)  sumre = sumre + (1.0/drr)*(1.0 - erf(sqrt(eta)*drr)); 
			}
		}
	}
	(*vijR) = sumre; 			
...
void    genpot(double *av1, double *av2, double *av3,  
  int natoms, int *atoms_num, double *atoms_charge,  double *atoms_xyz, double *J, 
  double *qeqrad, int *qeqids, int *qeqbasetype, int MaxEl, int *CalcMatrix)
{
      ...
      allocate_double_array(&(bgridx), (2*NUMR+1)*(2*NUMR+1)*(2*NUMR+1), "can not allocate bgridx\n"); 
      allocate_double_array(&(bgridy), (2*NUMR+1)*(2*NUMR+1)*(2*NUMR+1), "can not allocate bgridy\n"); 
      allocate_double_array(&(bgridz), (2*NUMR+1)*(2*NUMR+1)*(2*NUMR+1), "can not allocate bgridz\n"); 
      allocate_double_array(&(bgridcoef), (2*NUMR+1)*(2*NUMR+1)*(2*NUMR+1), "can not allocate bgridcoef\n"); 
      kvectors	= prepareKSUM(rradmax, eta, NUMR, bv1, bv2, bv3, bgridx, bgridy, bgridz, bgridcoef); 
     ...
     for (iatom = 0; iatom < natoms; iatom++) { 
		for (jatom = 0; jatom <= iatom; jatom++) {
			...
			/* printf("iatom=%d jatom=%d  vijre=%10.6f vijself=%10.6f vijR=%10.6f\n", 
		 	...
		 	if(! (iatom == jatom)) J[jatom*natoms+iatom] = 	J[iatom*natoms+jatom]; 
		}
     }	
...
/*			zeta_j = 0.5*qeqlambda*(2.0*jatom_nj+1.0)/qeqrad[atoms_num[jatom]-1];  */
			...
			if(atoms_num[jatom] == 1)  zeta_j = zeta_j + atoms_charge[jatom]/autoangs;  
			...
			for (t1 = -maxloop2[0]; t1 <= maxloop2[0]; t1++) { 
				for (t2 = -maxloop2[1];  t2 <= maxloop2[1]; t2++) { 
					for (t3 = -maxloop2[2]; t3 <= maxloop2[2]; t3++) {  
						...
						J[iatom*natoms+jatom] = J[iatom*natoms+jatom] + gamma - 1.0/dr; 
					}	
				}
			}	
			if(!(iatom == jatom)) J[jatom*natoms+iatom] = 	J[iatom*natoms+jatom]; 
...
void qmatrixelement_grid(double dx, double dy, double dz, double *av1, double *av2, double *av3, 
                             double avecszmin, double V, double eta,   double radmax, 
                             double *bgridx, double *bgridy, double *bgridz, double *bgridcoef, 
  ...
  int gridnl, double *pot, int *L1, int *L2, int *L3, double *h1, double *h2, double *h3)
{
     ...
     double avecszmin; 
      ...
      if (av2sz < avecszmin) avecszmin = av2sz; 
      if (av3sz < avecszmin) avecszmin = av3sz;
     #pragma omp parallel for private(ip,xx,yy,zz,iatom,dx,dy,dz,vijre,vijR) shared(gridnl,L1,L2,L3,h1,h2,h3,pot,natoms,atoms_xyz,av1,av2,av3,avecszmin,dvREAL,eta,radmax,bgridx,bgridy,bgridz,bgridcoef,kvectors,atoms_charge)
     ...
     for(ip = 0; ip < gridnl; ip++) {
	     ...
	     pot[ip] = 0.0;  	
			...
			qmatrixelement_grid(dx,dy,dz,av1,av2,av3, avecszmin, dvREAL, eta,  radmax, 
			                           bgridx, bgridy, bgridz, bgridcoef, kvectors, &vijre, &vijR); 
			pot[ip] = pot[ip] + (vijre + vijR)*atoms_charge[iatom]; 			   
		}
     }	

    /* potential is returned in eV */ 
     #pragma omp parallel for 	private(ip) shared(gridnl, pot, angstoev)
