...
void InitPointCharge(tPointCharge *pcptr, double *av1, double *av2, double *av3,
                     int natoms, double *atoms_xyz, char *fName) {
  int i;
  FILE *fp;
  ...
  if (fp != NULL) {
    ...
    allocate_double_array(&(pcptr->points_xyz), 3 * (pcptr->npoints),
                          "can not allocate points_xyz\n");
    allocate_double_array(&(pcptr->points_charge), pcptr->npoints,
                          "can not allocate points_charge\n");
    allocate_double_array(&(pcptr->atatom_pot), natoms,
                          "can not allocate atatom_pot\n");
    allocate_double_array(&(pcptr->atpoint_pot), pcptr->npoints,
                          "can not allocate atpoint_pot\n");
    ...
    for (i = 0; i < pcptr->npoints; i++) {
      ...
      printf("point charge %5d: %10.7f %10.7f %10.7f %10.7f\n", i + 1,
             pcptr->points_xyz[3 * i + 0], pcptr->points_xyz[3 * i + 1],
             pcptr->points_xyz[3 * i + 2], pcptr->points_charge[i]);
    }
  } else {
