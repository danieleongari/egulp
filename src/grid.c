...
void InitGrid(tGrid *gridptr, double res1, double res2, double res3,
  ...
  h1sz = sqrt((gridptr->gridh1)[0] * (gridptr->gridh1)[0] +
              (gridptr->gridh1)[1] * (gridptr->gridh1)[1] +
              (gridptr->gridh1)[2] * (gridptr->gridh1)[2]);
  h2sz = sqrt((gridptr->gridh2)[0] * (gridptr->gridh2)[0] +
              (gridptr->gridh2)[1] * (gridptr->gridh2)[1] +
              (gridptr->gridh2)[2] * (gridptr->gridh2)[2]);
  h3sz = sqrt((gridptr->gridh3)[0] * (gridptr->gridh3)[0] +
              (gridptr->gridh3)[1] * (gridptr->gridh3)[1] +
              (gridptr->gridh3)[2] * (gridptr->gridh3)[2]);

  printf("real space grid will be constructed....\n");
  printf(
      "%3d steps in A along lattice vector #1 %10.7f %10.7f %10.7f size "
      "%10.7f\n",
      gridptr->gridn1, gridptr->gridh1[0], gridptr->gridh1[1],
      gridptr->gridh1[2], h1sz);
  printf(
      "%3d steps in A along lattice vector #2 %10.7f %10.7f %10.7f size "
      "%10.7f\n",
      gridptr->gridn2, gridptr->gridh2[0], gridptr->gridh2[1],
      gridptr->gridh2[2], h2sz);
  printf(
      "%3d steps in A along lattice vector #3 %10.7f %10.7f %10.7f size "
      "%10.7f\n",
      gridptr->gridn3, gridptr->gridh3[0], gridptr->gridh3[1],
      gridptr->gridh3[2], h3sz);
  ...
  allocate_int_array(&(gridptr->gridLxyz_inv), ntot,
                     "can not allocate Lxyz_inv\n");
  ...
  for (i = 0; i < gridptr->gridn1; i++) {
    for (j = 0; j < gridptr->gridn2; j++) {
      for (k = 0; k < gridptr->gridn3; k++) {
        ...
        xx = i * (gridptr->gridh1)[0] + j * (gridptr->gridh2)[0] +
             k * (gridptr->gridh3)[0];
        yy = i * (gridptr->gridh1)[1] + j * (gridptr->gridh2)[1] +
             k * (gridptr->gridh3)[1];
        zz = i * (gridptr->gridh1)[2] + j * (gridptr->gridh2)[2] +
             k * (gridptr->gridh3)[2];
        for (iatom = 0; iatom < natoms; iatom++) {
          r1vdw = (vdw[atoms_num[iatom] - 1]) * (AUTOANG_GRID) *
          ...
          switch (gridptr->use_vdw_factor) {
            ...
            case 1: {
              r2vdw = (vdw[atoms_num[iatom] - 1]) * (AUTOANG_GRID) *
          ...
          for (icell = -1; icell <= 1; icell++) {
            for (jcell = -1; jcell <= 1; jcell++) {
              for (kcell = -1; kcell <= 1; kcell++) {
                xatom = (atoms_xyz)[iatom * 3 + 0] + icell * (av1[0]) +
                        jcell * (av2[0]) + kcell * (av3[0]);
                yatom = (atoms_xyz)[iatom * 3 + 1] + icell * (av1[1]) +
                        jcell * (av2[1]) + kcell * (av3[1]);
                zatom = (atoms_xyz)[iatom * 3 + 2] + icell * (av1[2]) +
                        jcell * (av2[2]) + kcell * (av3[2]);
                dist = sqrt((xx - xatom) * (xx - xatom) +
                            (yy - yatom) * (yy - yatom) +
                            (zz - zatom) * (zz - zatom));
                ...
                if (dist < r1vdw) {
                  gridptr
                      ->gridLxyz_inv[i * (gridptr->gridn2) * (gridptr->gridn3) +
                                     j * (gridptr->gridn3) + k] = 0;
                  goto out_label_now;
                }
              }
            }
        ...
        if (point_is_outside_flag == 1)
          gridptr->gridLxyz_inv[i * (gridptr->gridn2) * (gridptr->gridn3) +
                                j * (gridptr->gridn3) + k] = 0;
      out_label_now:;
      }
    }
  ...
  for (i = 0; i < gridptr->gridn1; i++) {
    for (j = 0; j < gridptr->gridn2; j++) {
      for (k = 0; k < gridptr->gridn3; k++) {
        if ((gridptr->gridLxyz_inv)[i * (gridptr->gridn2) * (gridptr->gridn3) +
                                    j * (gridptr->gridn3) + k] == 1)
          gridptr->gridnl = gridptr->gridnl + 1;
      }
    }
  }

  printf("resolution was set to %10.7f %10.7f %10.7f A\n",
         gridptr->resolution[0], gridptr->resolution[1],
         gridptr->resolution[2]);
  ...
  if (gridptr->use_vdw_factor == 0)
    printf("grid was constructed using fixed offset %10.6f\n", gridptr->offset);
  if (gridptr->use_vdw_factor == 1)
    printf("grid was constructed using final vdw factor %10.6f\n",
           gridptr->vdw_factor_f);
  printf("total number of points %d\n", ntot);
  printf("total number of simulation points %d\n", gridptr->gridnl);
  printf("allocating L1 L2 L3...\n");
  allocate_int_array(&(gridptr->gridL1), gridptr->gridnl,
                     "can not allocate gridL1\n");
  allocate_int_array(&(gridptr->gridL2), gridptr->gridnl,
                     "can not allocate gridL2\n");
  allocate_int_array(&(gridptr->gridL3), gridptr->gridnl,
                     "can not allocate gridL3\n");
...
void WriteGridAsCube(char *fName, tGrid grid, int natoms, int *atoms_num,
  ...
  fprintf(fp, "resolution(A) %10.6f %10.6f %10.6f\n", grid.resolution[0],
          grid.resolution[1], grid.resolution[2]);
  fprintf(fp,
          "vdw_factor_i %10.6f  vdw_factor_f %10.6f  filename %s simpoints %d "
          "use_vdw_factor %d offset %10.6f\n",
          grid.vdw_factor_i, grid.vdw_factor_f, fName, grid.gridnl,
          grid.use_vdw_factor, grid.offset);
  fprintf(fp, "%6d %11.6f %11.6f %11.6f\n", natoms, 0.0, 0.0, 0.0);
  fprintf(fp, "%6d %11.6f %11.6f %11.6f\n", grid.gridn1,
          grid.gridh1[0] / (AUTOANG_GRID), grid.gridh1[1] / (AUTOANG_GRID),
          grid.gridh1[2] / (AUTOANG_GRID));
  fprintf(fp, "%6d %11.6f %11.6f %11.6f\n", grid.gridn2,
          grid.gridh2[0] / (AUTOANG_GRID), grid.gridh2[1] / (AUTOANG_GRID),
          grid.gridh2[2] / (AUTOANG_GRID));
  fprintf(fp, "%6d %11.6f %11.6f %11.6f\n", grid.gridn3,
          grid.gridh3[0] / (AUTOANG_GRID), grid.gridh3[1] / (AUTOANG_GRID),
  ...
  for (ixi = 0; ixi < grid.gridn1; ixi++) {
    for (iyi = 0; iyi < grid.gridn2; iyi++) {
      for (izi = 0; izi < grid.gridn3; izi++) {
        ifun_val = grid.gridLxyz_inv[ixi * (grid.gridn2) * (grid.gridn3) +
                                     iyi * (grid.gridn3) + izi];
        if (ifun_val == 1)
          sprintf(str, "%13.6E ", VALIDPOINT);
        else
          sprintf(str, "%13.6E ", INVALIDPOINT);
        fprintf(fp, str);
...
void InitGridFromCube(char *fName, tGrid *newgridptr, int natoms) {
  ...
  allocate_int_array(&(atoms_num), natoms,
                     "can not allocate atoms_num in InitGridFromCube\n");
  allocate_double_array(&(atoms_xyz), 3 * natoms,
                        "can not allocate atoms_xyz in InitGridFromCube\n");
  ...
  printf("resolution (A)   %10.6f %10.6f %10.6f\n", newgridptr->resolution[0],
         newgridptr->resolution[1], newgridptr->resolution[2]);
  /*   fprintf(fp, "vdw_factor_i %10.6f  vdw_factor_f %10.6f  filename %s
   * simpoints %d use_vdw_factor %d offset %10.6f\n", grid.vdw_factor_i,
   * grid.vdw_factor_f,  fName, grid.gridnl, grid.use_vdw_factor, grid.offset);
   */
  fscanf(fp, "%s %s %s %s %s %s %s %s %s %s %s %s\n", tmpstr1, tmpstr2, tmpstr3,
         tmpstr4, tmpstr5, tmpstr6, tmpstr7, tmpstr8, tmpstr9, tmpstr10,
         tmpstr11, tmpstr12);
  ...
  printf("use_vdw_factor = %d offset = %10.6f\n", newgridptr->use_vdw_factor,
         newgridptr->offset);
  /* read number of atoms and origin */
  ReadPATTERNSTR1(&fp, tmpstr1, tmpstr2, tmpstr3, tmpstr4, &NumOfAtoms, &cxx,
                  &cyy, &czz);
  /* read number of points in e_1, e_2, e_3 and unit vectors */
  ReadPATTERNSTR1(&fp, tmpstr1, tmpstr2, tmpstr3, tmpstr4,
                  &(newgridptr->gridn1), &(newgridptr->gridh1[0]),
                  &(newgridptr->gridh1[1]), &(newgridptr->gridh1[2]));
  ReadPATTERNSTR1(&fp, tmpstr1, tmpstr2, tmpstr3, tmpstr4,
                  &(newgridptr->gridn2), &(newgridptr->gridh2[0]),
                  &(newgridptr->gridh2[1]), &(newgridptr->gridh2[2]));
  ReadPATTERNSTR1(&fp, tmpstr1, tmpstr2, tmpstr3, tmpstr4,
                  &(newgridptr->gridn3), &(newgridptr->gridh3[0]),
                  &(newgridptr->gridh3[1]), &(newgridptr->gridh3[2]));
  ...
  newgridptr->dv3 =
      cellVolume(newgridptr->gridh1, newgridptr->gridh2, newgridptr->gridh3);
  ...
  h1sz = sqrt((newgridptr->gridh1)[0] * (newgridptr->gridh1)[0] +
              (newgridptr->gridh1)[1] * (newgridptr->gridh1)[1] +
              (newgridptr->gridh1)[2] * (newgridptr->gridh1)[2]);
  h2sz = sqrt((newgridptr->gridh2)[0] * (newgridptr->gridh2)[0] +
              (newgridptr->gridh2)[1] * (newgridptr->gridh2)[1] +
              (newgridptr->gridh2)[2] * (newgridptr->gridh2)[2]);
  h3sz = sqrt((newgridptr->gridh3)[0] * (newgridptr->gridh3)[0] +
              (newgridptr->gridh3)[1] * (newgridptr->gridh3)[1] +
              (newgridptr->gridh3)[2] * (newgridptr->gridh3)[2]);
  printf("full_lines=%d rem=%d\n", full_lines, rem);
  printf("%3d steps along lattice vector #1 %10.7f %10.7f %10.7f size %10.7f\n",
         newgridptr->gridn1, newgridptr->gridh1[0], newgridptr->gridh1[1],
         newgridptr->gridh1[2], h1sz);
  printf("%3d steps along lattice vector #2 %10.7f %10.7f %10.7f size %10.7f\n",
         newgridptr->gridn2, newgridptr->gridh2[0], newgridptr->gridh2[1],
         newgridptr->gridh2[2], h2sz);
  printf("%3d steps along lattice vector #3 %10.7f %10.7f %10.7f size %10.7f\n",
         newgridptr->gridn3, newgridptr->gridh3[0], newgridptr->gridh3[1],
         newgridptr->gridh3[2], h3sz);
  ntot = (newgridptr->gridn1) * (newgridptr->gridn2) * (newgridptr->gridn3);
  printf("total number of points is %d\n", ntot);
  printf("volume element is %10.7f\n", newgridptr->dv3);

  allocate_int_array(&(newgridptr->gridLxyz_inv), ntot,
                     "can not allocate Lxyz_inv\n");
  /* printf("entering the loop...\n"); */
  newgridptr->gridnl = 0;
  for (ixi = 0; ixi < newgridptr->gridn1; ixi++) {
    for (iyi = 0; iyi < newgridptr->gridn2; iyi++) {
      for (izi = 0; izi < newgridptr->gridn3; izi++)
        newgridptr
            ->gridLxyz_inv[ixi * (newgridptr->gridn2) * (newgridptr->gridn3) +
                           iyi * (newgridptr->gridn3) + izi] = 0;
      for (i = 0; i < full_lines; i++) {
        fscanf(fp, PATTERNSTR3, &(tmpstr_array[0][0]), &(tmpstr_array[1][0]),
               &(tmpstr_array[2][0]), &(tmpstr_array[3][0]),
               &(tmpstr_array[4][0]), &(tmpstr_array[5][0]));
        for (j = 0; j < PATTERN; j++)
          if (atof(&(tmpstr_array[j][0])) == VALIDPOINT) {
            newgridptr
                ->gridLxyz_inv[ixi * (newgridptr->gridn2) *
                                   (newgridptr->gridn3) +
                               iyi * (newgridptr->gridn3) + i * PATTERN + j] =
                1;
            (newgridptr->gridnl)++;
          }
      }
      ...
      switch (rem) {
        ...
        case 1:
          ...
          if (atof(tmpstr1) == VALIDPOINT) {
            newgridptr->gridLxyz_inv[ixi * (newgridptr->gridn2) *
                                         (newgridptr->gridn3) +
                                     iyi * (newgridptr->gridn3) +
                                     full_lines * PATTERN + 0] = 1;
            (newgridptr->gridnl)++;
          }
          break;
        case 2:
  ...
  printf("total number of simulation points %d\n", newgridptr->gridnl);
  allocate_int_array(&(newgridptr->gridL1), newgridptr->gridnl,
                     "can not allocate gridL1\n");
  allocate_int_array(&(newgridptr->gridL2), newgridptr->gridnl,
                     "can not allocate gridL2\n");
  allocate_int_array(&(newgridptr->gridL3), newgridptr->gridnl,
                     "can not allocate gridL3\n");
  ...
  for (i = 0; i < newgridptr->gridn1; i++) {
    for (j = 0; j < newgridptr->gridn2; j++) {
      for (k = 0; k < newgridptr->gridn3; k++) {
        if (newgridptr
                ->gridLxyz_inv[i * (newgridptr->gridn2) * (newgridptr->gridn3) +
                               j * (newgridptr->gridn3) + k] > 0) {
